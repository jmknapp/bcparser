...
int main(int argc, char **argv) {
	...
	for (j = 0 ; j < blk->transactionCounter ; j++) {
	    ...
	    if (btc*btcprice > 10000000) {
	        fprintf(stderr, "%s %u %s %.6f $%.1f million\n", blkfile, blk->blkhdr->timestamp, bufstr(blk->transactions[j].hash, HASHLEN, true), btc, btc*btcprice/1e6) ;
		sprintf(query,"insert into btcwhales (utime,txid,btc,usdsize) VALUES(%u, \"%s\", %f, %f)", blk->blkhdr->timestamp, bufstr(blk->transactions[j].hash, HASHLEN, true), btc, btc*btcprice/1e6) ;
